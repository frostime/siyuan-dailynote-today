...
* 如果你需要按「日期 × 笔记本」浏览过去的 Daily Note，可以使用 **Daily Note View**。它会先检查日记是否存在，浏览缺失日期不会自动创建文档。
...
从顶栏菜单点击「打开 Daily Note View」可以进入一个独立的日记视图，用来浏览不同日期和不同笔记本中的 Daily Note。

* 支持「今日」「三日」「笔记本」「周」「月」几种视图
* 多栏笔记视图会在一个页签中横向展示多个日期或多个笔记本，避免反复打开普通文档页签
* 周/月视图用于查看日记状态：周视图按笔记本 × 日期显示，月视图在日期格中列出已有日记的笔记本
* 浏览缺失日记不会自动创建文档；只有点击「创建 Daily Note」按钮才会写入
* 如果同一个日期和笔记本存在重复日记，会显示重复提示并提供打开重复文档的入口
* 可以在设置 → 日记中调整 Daily Note View 多栏最小宽度
...
2. 插件会自动匹配块内容中的日期时间，确定后，插件会帮你把这份预约记录下来
    ...
    ​![](asset/Reserve2.png)​
3. Update On 2025-01-24：v1.8.0 版本之后，你可以在这个对话框当中手动选择想要预约的日期

    ​![image](asset/image-20250124221536-wal1qgl.png)​
4. 等到当天的时候，插件在自动创建完日记后，会帮你把相关的预约以嵌入块的形式插入日记中

    ​![](asset/Reserve3.png)​
5. 如果你发现预约的嵌入块没有自动插入，可以点击图标菜单中的「更新」按钮更新文档
6. 完成预约块后，会自动为块添加一个自定义属性 `custom-reservation`​ 并填写备注，方便用户快速识别哪些块加入了预约

    ​![](asset/R-memo.png)​

    ​![](asset/R-attr.png)​
7. 再次点击一个已经预约过的块，你可以选择「取消预约」
8. 默认将所有的预约作为一个嵌入块插入到文档开头，如果你对默认行为不满，可以在设置中自行更改。
